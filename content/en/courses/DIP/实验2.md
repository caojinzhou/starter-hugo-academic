...
im_a0 = imfilter(im_noise_salt);
im_a1 = imfilter(im_noise_salt, h, 'replicate');
im_a2 = imfilter(im_noise_salt, h, 'x');
im_a3 = imfilter(im_noise_salt, h, 0);
im_a4 = imfilter(im_noise_salt, h, 'symmetric');
im_a5 = imfilter(im_noise_salt, h, 'circular');
...
1,1,1];
K=conv2(T,w,'same');
subplot(1,2,2),imshow(K);title('拉普拉斯滤波后图像');
```
...
3)	分别采用5*5、9*9、 15*15 和 25*25 大小的拉普拉斯算子对图像进行锐化滤波，并g(x,y)=f(x,y)-∇^2 f(x,y)完成图像的锐化增强，观察其有何不同，要求在同一窗口中显示。
